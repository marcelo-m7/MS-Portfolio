...
const Particles = ({ visibleRef }: { visibleRef: MutableRefObject<boolean> }) => {
  ...
  return (
    <points ref={pointsRef} geometry={geometry}>
      <pointsMaterial
        attach="material"
        color="#0EA5E9" // Using hex for --secondary
...
const Ribbon = ({ visibleRef }: { visibleRef: MutableRefObject<boolean> }) => {
  ...
  return (
    <mesh ref={meshRef} scale={1.65} position={[0, 0, 0]}>
      ...
      <MeshDistortMaterial
        color="#7C3AED" // Using hex for --primary
        emissive="#EC4899" // Using hex for --accent
    ...
    <div className="pointer-events-none absolute inset-0 -z-10">
      ...
      >
        <Suspense fallback={null}>
          ...
          <directionalLight position={[4, 5, 6]} intensity={1.1} color="#7C3AED" /> {/* Using hex for --primary */}
          <pointLight position={[-6, -4, -4]} intensity={0.8} color="#0EA5E9" /> {/* Using hex for --secondary */}
